...
// Normalize angle difference to be within [-π, π]
...
int main() {
    double i = 5, j = 4, k = 2;  // Adjust penalty factors and heuristic scaling
    ...
    while (!minHeap.empty()) {
        ...
        for (auto& dir : directions) {
            ...
            if (nx >= 0 && nx < 10 && ny >= 0 && ny < 10 && map[nx][ny] == 0 && visited[nx][ny] == 0) {
                ...
                // Normalize the angle difference and calculate momentum penalty
                double momentumPenalty = abs(normalizeAngleDifference(momentumMap[x][y], newAngle));

                // Apply momentum penalty scaling
                ...
                if (newCost < seen[nx][ny]) {
                    momentumMap[nx][ny] = newAngle;  // Store the new angle (momentum)
                    seen[nx][ny] = newCost;  // Record the new cost
