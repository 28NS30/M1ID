...
void train(vector<vector<double>>inputs, vector<vector<double>> trueYs){
    vector<vector<double>> dError(weights.size()); //derivative of the error with respect to the pre-actavtion weighted sum of a node
    // OUTPUT LAYER
        ...
        for (int dataPoint = 0; dataPoint < predictions.size(); dataPoint++){
            ...
            for (int layer = weights.size()-1; layer>=0; layer--){
                ...
                if (layer == weights.size()-1){
                    // OUTPUT LAYER
                    for (int node = 0; node < weights[layer].size(); node++){
                        grad = (predictions[dataPoint][node] - trueYs[dataPoint][node])*(predictions[dataPoint][node] * (1 - predictions[dataPoint][node]));
                        dError[weights.size() - 1 - layer].push_back(grad);
                        for (int weight = 0; weight < weights[layer][node].size(); weight++){
                                // Update Weight
                                weights[layer][node][weight] -= eta*grad;
                            }
                            // Update Bias
                    ...
                    for (int node = 0; node < weights[layer].size(); node++){
                        for (int weight = 0; weight < weights[layer][node].size(); weight++){
                            // Update Weight
                            weights[layer][node][weight] -= eta*dError[layer][node];
                        }
                        // Update Bias
